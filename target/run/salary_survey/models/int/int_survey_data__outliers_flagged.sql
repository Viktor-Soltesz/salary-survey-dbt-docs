  ...
  create or replace view `software-developer-salaries`.`surveys`.`int_survey_data__outliers_flagged`
  OPTIONS(
      description="""Flags outlier survey responses using a robust z-score method based on the modified z-score formula. Uses the median and median absolute deviation (MAD) to compute a robust outlier indicator.\n"""
    )
  as -- int_survey_data__outliers_flagged.sql
